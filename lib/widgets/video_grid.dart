      ...
      for (var i = 0; i < remoteUids.length; i++) {
        tiles.add(
          _VideoTile(
            label: 'Student ${i + 1}',
            subtitle: 'Connected',
            child: _buildRemoteView(remoteUids[i]),
          ),
        );
      }
    }
        ...
        children: [
          Container(
            decoration: BoxDecoration(
              ...
              gradient: isPlaceholder
                  ? LinearGradient(
                      colors: [AppPalette.wash, AppPalette.surface],
                    )
                  : null,
            ),
            child: child,
          ...
          if (muted || cameraOff)
            Positioned(
              ...
              child: Row(
                children: [
                  if (muted) const _StatusIcon(icon: Icons.mic_off_outlined),
                  if (cameraOff)
                    const Padding(
                      padding: EdgeInsets.only(left: 6),
                      child: _StatusIcon(icon: Icons.videocam_off_outlined),
                    ),
                ],
              ),
            ),
...
class _PlaceholderContent extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Center(child: Icon(icon, color: AppPalette.muted, size: 48));
  }
}
